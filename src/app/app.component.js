"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var gold_price_service_service_1 = require("./services/gold-price-service.service");
var AppComponent = (function () {
    function AppComponent(goldPriceService) {
        this.goldPriceService = goldPriceService;
    }
    AppComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.goldPriceService.getTodayGoldPrice()
            .subscribe(function (goldPrice) {
            return _this.updateView(goldPrice);
        });
    };
    AppComponent.prototype.updateView = function (goldPrice) {
        this.goldPrice = goldPrice;
    };
    return AppComponent;
}());
AppComponent = __decorate([
    core_1.Component({
        selector: 'my-app',
        template: "<h1>Price list NBP</h1>    \n  <nav>\n    <a routerLink=\"/home\" routerLinkActive=\"active\">Home</a>\n    <a routerLink=\"/goldprice\" routerLinkActive=\"active\">Gold prices</a>\n    <a routerLink=\"/tablea\" routerLinkActive=\"active\">Tabela A prices</a>\n  </nav>\n  <router-outlet></router-outlet>\n  ", styleUrls: ['./app.component.css']
    }),
    __metadata("design:paramtypes", [gold_price_service_service_1.GoldPriceService])
], AppComponent);
exports.AppComponent = AppComponent;
//# sourceMappingURL=app.component.js.map