...
describe('Table A service integration test', () => {
  ...
  it('should return the currency with the lowest price', async(inject(
    [TableAService, MockBackend], (service: TableAService, mockBackend: MockBackend) => {
      /*given*/
      mockBackend.connections.subscribe((connection: MockConnection) => {
        const goldPrice: Object[] = [
      ...
      /*when*/
      const result: Observable<Rate> = service.getCurrencyWithTheLowestCurrentRate();

      /*then*/
      result.subscribe((res) => expect(res.mid).toEqual(0.10)
        , (error: any) => fail());
    })));
