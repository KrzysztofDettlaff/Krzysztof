...
export class TableAService {
 ...
 getCurrencyWithTheHighestCurrentRate(): Observable<Rate> {
  ...
  getCurrencyWithTheLowestCurrentRate(): Observable<Rate> {
    return this.tableAHttpClientService.getRateList().map((rates) => this.tableACalculatorService.getMinRate(rates));
  }
